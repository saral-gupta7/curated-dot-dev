...
Content is organized under `src/constants/`: topic navigation in `topicGroups.ts`, topic-grouped video files in `videoResources/`, topic-grouped article files in `articleResources/`, shared types in `constants.types.ts`, and taxonomy utilities in `topicHelpers.ts`.
